...
class Admin_Notices {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_dismiss_admin_notice_' . Plugin::instance()->basename, array( __CLASS__, 'handle_dismiss_notice' ) );
	}
	...
	public static function render_notice( $notice ) {
		$output            = '';
		$plugin_class      = Helper::dasherize( Plugin::instance()->id );
		$dismissible_class = $notice['dismissible'] ? 'is-dismissible' : '';
		$nonce             = $notice['dismissible'] ? wp_create_nonce( Plugin::instance()->id . '_dismiss_notice' ) : '';
		...
		$output .= sprintf( '<div class="notice %2$s %3$s %4$s %5$s %7$s-admin-notice" data-notice-id="%5$s" data-plugin-id="%6$s" data-nonce="%8$s">%1$s</div>',
			...
			esc_attr( $notice['id'] ),
			esc_attr( Plugin::instance()->basename ),
	...
	public static function handle_dismiss_notice() {
		...
		$nonce_action = Plugin::instance()->id . '_dismiss_notice';

		if ( empty( $notice_id ) || $plugin_id !== esc_attr( Plugin::instance()->basename ) || ! wp_verify_nonce( $nonce, $nonce_action ) ) {
