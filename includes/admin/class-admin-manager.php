...
class Admin_Manager {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_product_data_tabs', array( __CLASS__, 'grouped_product_settings_tabs'));
		add_action( 'woocommerce_product_data_panels', array( __CLASS__, 'write_data_panels' ) );
	...
	/**
	 * Add new tab for grouped product data panels
	 *
	 ...
	 * @since 1.1.1
	*/
	public static function grouped_product_settings_tabs( $tabs ) {
		$tabs['min_max'] = array(
			'label'  => __( 'Min max settings', 'wc-min-max-quantities' ),
			'target' => 'min_max_product_data',
	 ...
	 * @return void
	 */
	public static function write_data_panels() {
		global $post;
		echo '<div id="min_max_product_data" class="panel woocommerce_options_panel hidden">';
		?>
			<?php
			...
			$settings = get_post_meta( $post->ID, '_wc_min_max_quantities_group_override', true );
			$css      = 'yes' === $settings ? '' : 'display:none;';
			echo '<div class="wc-min-max-override-settings" style="' . esc_attr( $css ) . '">';
